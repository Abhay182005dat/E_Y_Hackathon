...
const {
  updateWithVersion,
  updateWithRetry,
} = require("./server/utils/optimisticLock");
...
(async () => {
  try {
    ...
    try {
      ...
      console.warn(
        "⚠️ Redis not available. Chat will use fallback mode:",
        redisErr.message,
      );
    ...
    try {
      ...
      console.warn(
        "⚠️ Blockchain not available. Audit trail will use JSON fallback:",
...
app.post("/api/auth/send-otp", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[Server.js] About to call sendOTP with: name=${name}, accountNumber=${accountNumber}, phone=${phone}`,
    );
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    console.log(
      `[Auth] Login attempt: phone=${phone}, accountNumber=${accountNumber}`,
    );
    const { token, user } = await verifyLoginOTP({ phone, otp, accountNumber });
    console.log(
      `✅ [Auth] Login successful: userId=${user._id || user.phone}, email=${user.email}`,
    );
...
app.post("/loan", async (req, res) => {
  ...
  try {
    ...
    const userId =
      userData.phone || userData.accountNumber || userData.userId || "unknown";
    ...
    if (intent !== "loanApplication") {
      return res
        .status(400)
        .json({ status: "rejected", reason: "Invalid intent" });
    }

    // PHASE 2: Data Collection & Consent
    const { consentCid, interactionCid } = await collectUserData(
      sessionId,
      userData,
    );
    cids.push(
    ...
    const { kycStatus, reason: kycReason } = await verifyKYC(
      sessionId,
    ...
    if (kycStatus !== "verified") {
      return res
        .status(200)
        .json({ status: "rejected", reason: kycReason || "KYC failed", cids });
    }
    ...
    if (!creditCheck.riskAcceptable) {
      return res
        ...
        .json({
          status: "rejected",
          reason: creditCheck.reason || "Credit risk not acceptable",
          cids,
        });
    }

    // PHASE 4 (cont.): Underwriting
    const underwriting = await evaluateRiskAndPrice(
      sessionId,
      userData,
      creditCheck.creditData,
    );
    cids.push({ step: "underwriting", cid: underwriting.cid });
    if (!underwriting.eligibility) {
      return res
        .status(200)
        .json({ status: "rejected", reason: underwriting.reason, cids });
    }

    // PHASE 5: Loan Offer Generation & Negotiation
    const finalOffer = await presentAndNegotiateOffer(
      sessionId,
      underwriting.offer,
    );
    ...
    if (finalOffer.userResponse !== "accepted") {
      return res
        .status(200)
        .json({ status: "rejected", reason: "User rejected the offer", cids });
    }

    // PHASE 6: Loan Approval
    const approvalDetails = await executeApproval(
      sessionId,
      kycStatus,
      creditCheck,
      finalOffer,
    ...
    if (approvalDetails.approvalStatus !== "approved") {
      return res
        .status(200)
        .json({ status: "rejected", reason: approvalDetails.reason, cids });
    }
    ...
    const { disbursementCid } = await disburseFunds(
      sessionId,
      loanId,
      { ...finalOffer, userId },
      userId,
      ...
      setTimeout(async () => {
        try {
          ...
          if (masterResult.success) {
            console.log(
              `✅ Master contract uploaded to IPFS: ${masterResult.ipfsHash}`,
            );
...
app.post("/payment", async (req, res) => {
  try {
    ...
    const userId =
      ...
      paymentData.userId ||
      (req.user ? req.user.phone || req.user._id?.toString() : "unknown");
  ...
  async (req, res) => {
    try {
      ...
      if (files.aadhaar?.[0]) {
        ...
        console.log(`\n📊 PARSED DATA:`);
        console.log(
          `Aadhaar Number: ${results.aadhaar.aadhaar || "Not found"}`,
        );
        ...
        console.log(
          `Confidence: ${(results.aadhaar.confidence * 100).toFixed(0)}%`,
        );
        console.log(`${"=".repeat(70)}\n`);
      }
      if (files.pan?.[0]) {
        ...
        console.log(
          `Valid Format: ${results.pan.isValidFormat ? "YES ✅" : "NO ❌"}`,
        );
        console.log(
          `Confidence: ${(results.pan.confidence * 100).toFixed(0)}%`,
        );
        console.log(`${"=".repeat(70)}\n`);
      }
      if (files.bankStatement?.[0]) {
        results.bankStatement = await parseBankStatement(
        ...
        console.log(`\n📊 PARSED DATA:`);
        console.log(
          `Account Number: ${results.bankStatement.accountNumber || "Not found"}`,
        );
        console.log(
          `Account Holder: ${results.bankStatement.accountHolderName || "Not found"}`,
        );
        console.log(`Bank: ${results.bankStatement.bankName || "Not found"}`);
        console.log(`IFSC: ${results.bankStatement.ifscCode || "Not found"}`);
        console.log(
          `Estimated Salary: ₹${results.bankStatement.estimatedMonthlySalary || "N/A"}`,
        );
        console.log(
          `Confidence: ${(results.bankStatement.confidence * 100).toFixed(0)}%`,
        );
        console.log(`${"=".repeat(70)}\n`);
      }
      if (files.salarySlip?.[0]) {
        ...
        console.log(`\n📊 PARSED DATA:`);
        console.log(
          `Employee Name: ${results.salarySlip.employeeName || "Not found"}`,
        );
        console.log(
          `Basic Salary: ₹${results.salarySlip.basicSalary || "N/A"}`,
        );
        console.log(
          `Gross Salary: ₹${results.salarySlip.grossSalary || "N/A"}`,
        );
        console.log(`Net Salary: ₹${results.salarySlip.netSalary || "N/A"}`);
        console.log(
          `Confidence: ${(results.salarySlip.confidence * 100).toFixed(0)}%`,
        );
        console.log(`${"=".repeat(70)}\n`);
      }
      ...
      if (results.pan) {
        ...
        console.log(`   Name: ${results.pan.name || "Not found"}`);
        console.log(
          `   Father's Name: ${results.pan.fatherName || "Not found"}`,
        );
      ...
      if (results.bankStatement) {
        ...
        console.log(
          `   Bank Name: ${results.bankStatement.bankName || "Not found"}`,
        );
        console.log(
          `   IFSC Code: ${results.bankStatement.ifscCode || "Not found"}`,
        );
        console.log(
          `   Estimated Monthly Salary: ₹${results.bankStatement.estimatedMonthlySalary || "N/A"}`,
        );
        console.log(
          `   Confidence: ${(results.bankStatement.confidence * 100).toFixed(0)}%`,
        );
      ...
      console.log(
        `   Total Documents Processed: ${Object.keys(results).length}`,
      );
...
app.post("/api/calculate-score", async (req, res) => {
  try {
    ...
    console.log(
      `Documents Provided: ${Object.keys(documents).join(", ") || "None"}`,
    );
    ...
    console.log(
      `   Eligible for Loan: ${score.eligibleForLoan ? "YES ✅" : "NO ❌"}`,
    );
    if (score.breakdown) {
      console.log(`\n   Component Scores:`);
      Object.entries(score.breakdown).forEach(([key, value]) => {
        console.log(
          `     - ${key}: ${value.score}/${value.maxScore} (weight: ${value.weight}%)`,
        );
      });
    }
    console.log(`\n💳 PRE-APPROVED LIMIT:`);
...
app.post("/api/chat", async (req, res) => {
  try {
    const { message, customerData, creditScore, sessionId, documents } =
      req.body;
    ...
    if (!session) {
      ...
      console.log(
        `[Chat] Session restored: ${sid} | State: ${session.state} | Negotiation: ${session.negotiationCount}`,
      );
    }
    ...
    const score =
      ...
      650; // Use 650 (min eligible) as fallback
    ...
    const finalAmount =
      requestedAmount <= preApprovedLimit ? requestedAmount : preApprovedLimit;
    ...
    if (session.state === "accepted") {
      ...
      if (session.state === "negotiating" || session.state === "offered") {
        ...
        const statusMessage =
          requestedAmount <= preApprovedLimit
            ? "✅ Approved"
      ...
      const amountMatch = message.match(
        /(\d+(?:,\d+)*(?:\.\d+)?)\s*(lakh|lac|thousand|lakhs|lacs)?\b/i,
      );
      let newAmount = null;
      ...
      if (amountMatch) {
        ...
        const shortUnitMatch = message.match(
          /(\d+(?:,\d+)*(?:\.\d+)?)\s*([lLkK])\b/,
        );
      ...
      (session.state === "offered" || session.state === "negotiating") &&
      /^\s*[\d,.\s]+\s*(lakh|lac|lakhs|lacs|l|k|thousand)?\s*$/i.test(lower)
    ) {
      // Plain number input during active session - treat as amount change
      const numMatch = message.match(
        /(\d+(?:,\d+)*(?:\.\d+)?)\s*(lakh|lac|lakhs|lacs|thousand)?\b/i,
      );
      let newAmount = null;
    ...
    if (session.state === "accepted" && !session.applicationStored) {
      ...
      const userId =
        customerData?.phone || customerData?.accountNumber || "N/A";
      ...
      if (blockchainInitialized) {
        try {
          console.log(
            `🔗 [Blockchain] Starting transaction logging for ${userId}...`,
          );
          ...
          console.log(
            `  [1/6] Application: ${appResult.success ? "✅" : "❌"} ${appResult.transactionHash || appResult.error || ""}`,
          );
          ...
          console.log(
            `  [2/6] PAN: ${panResult.success ? "✅" : "❌"} ${panResult.transactionHash || panResult.error || ""}`,
          );
          await delay(1500);
          ...
          await delay(1500);
          console.log(
            `  [3/6] Aadhaar: ${aadhaarResult.success ? "✅" : "❌"} ${aadhaarResult.transactionHash || aadhaarResult.error || ""}`,
          );
          ...
          await delay(1500);
          console.log(
            `  [4/6] Credit: ${creditResult.success ? "✅" : "❌"} ${creditResult.transactionHash || creditResult.error || ""}`,
          );
          ...
          await delay(1500);
          console.log(
            `  [5/6] Disbursement: ${disburseResult.success ? "✅" : "❌"} ${disburseResult.transactionHash || disburseResult.error || ""}`,
          );
          ...
          console.log(
            `  [6/6] EMI: ${emiResult.success ? "✅" : "❌"} ${emiResult.transactionHash || emiResult.error || ""}`,
          );
          await delay(1500);
          ...
          console.log(
            `  [+] Chat: ${chatResult.success ? "✅" : "❌"} ${chatResult.transactionHash || chatResult.error || ""}`,
          );
          ...
          console.log(
            `📄 [Hybrid] Generating master contract immediately using local data...`,
          );
          const localData = {
            application: {
              _id: app._id,
          ...
          const masterResult = await generateAndUploadMasterContract(
            userId,
            localData,
          );
          if (masterResult.success) {
            console.log(
              `✅ Master contract (hybrid) uploaded to IPFS: ${masterResult.ipfsHash}`,
            );
            console.log(`   📂 View at: ${masterResult.ipfsUrl}`);
            ...
            try {
              ...
              if (updateResult.modifiedCount === 0) {
                console.log(
                  "ℹ️  IPFS link not updated (status changed or document modified). Attempting unguarded update...",
                );
                try {
                  const forceUpdate = await db
                    .collection("applications")
                    .updateOne(
                      { _id: app._id },
                      {
                  ...
                  if (forceUpdate.modifiedCount > 0) {
                    console.log(
                      "✅ IPFS link + tx hashes force-updated successfully",
                    );
                  } else {
                    console.log(
                      "⚠️  Force update did not modify document (maybe identical values already present)",
                    );
                  }
                } catch (e) {
                  console.error(
                    "⚠️  Failed force-updating IPFS link:",
                    e.message,
                  );
                }
              } else {
            ...
            console.error(
              "⚠️  Master contract generation failed:",
              masterResult.error,
            );
          }
        } catch (err) {
      ...
      console.log(
        `✅ [Application] Stored: ${app._id} for ${name} | Requested: ₹${requestedAmount} | Approved: ₹${approvedAmount} | EMI: ₹${emiData.emi}`,
      );
    }
    ...
    if (blockchainInitialized) {
      const userId =
        customerData?.phone || customerData?.accountNumber || "unknown";
    ...
    console.log(
      `[Chat] ${name} | State: ${session.state} | Negotiation: ${session.negotiationCount}`,
    );
...
app.post("/api/chat/restore", async (req, res) => {
  try {
    ...
    const history = redisInitialized
      ? await getChatHistory(sessionId, 100)
      : [];
    ...
    console.log(
      `[Session Restore] ${sessionId} | State: ${session.state} | Messages: ${history.length}`,
    );
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `[Update] Attempting to update application: ${id} -> ${status}`,
      );
      ...
      if (exists.status === status) {
        console.log(
          `[Update] Application ${id} already in status '${status}', skipping update`,
        );
        return res.json({
          ok: true,
      ...
      if (!result.success) {
        if (result.conflict) {
          return res.status(409).json({
            error:
              "Conflict: Another admin modified this application. Please refresh.",
      ...
      console.log(
        `✅ [Admin:${req.user.email}] Application ${id}: ${status} (v${result.newVersion})`,
      );
  ...
  async (req, res) => {
    try {
      ...
      if (!["approved", "rejected"].includes(status)) {
        return res
          .status(400)
          .json({ error: "Invalid status for batch update" });
      }
      ...
      console.log(
        `✅ [Batch:${req.user.email}] Updated ${results.success.length}/${applicationIds.length}`,
      );
...
app.get(
  "/api/blockchain/user/:userId/ledger",
  authMiddleware,
  async (req, res) => {
    try {
...
app.get(
  "/api/blockchain/user/:userId/history",
  authMiddleware,
  async (req, res) => {
    try {
      const { userId } = req.params;
...
app.post(
  "/api/blockchain/user/:userId/master-contract",
  authMiddleware,
  async (req, res) => {
    try {
      const { userId } = req.params;
      ...
      if (regenerate) {
        ...
        if (application && application.blockchainTxHashes) {
          console.log(
            `📄 [Regenerate] Using HYBRID mode for ${userId} (tx hashes found)`,
          );
          ...
          const localData = {
            ...
            creditScore: {
              ...
              grade:
                (application.approvalScore || 0) >= 750
                  ? "A+"
                  : (application.approvalScore || 0) >= 700
                    ? "A"
                    : (application.approvalScore || 0) >= 650
                      ? "B"
                      : "C",
            },
            emiData: {
          ...
          console.log(
            `📄 [Regenerate] Using LEGACY mode for ${userId} (no tx hashes - querying blockchain)`,
          );
        ...
        if (result.success && application) {
          try {
            ...
            console.log(
              `✅ [Regenerate] Updated application ${application._id} with new IPFS hash`,
            );
          } catch (updateErr) {
            console.error(
              "⚠️  Failed to update application with new IPFS hash:",
              updateErr.message,
            );
          }
        }
...
app.listen(port, () => {
  ...
  console.log(
    `💬 Chat: ${redisInitialized ? "Redis Streams enabled" : "Fallback mode (no Redis)"}`,
  );
  console.log(
    `🔗 Blockchain: ${blockchainInitialized ? "Ethereum ledger active (immutable audit)" : "JSON fallback mode"}`,
  );
});
